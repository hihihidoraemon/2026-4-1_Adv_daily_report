...
GITHUB_TEMPLATE_URL = "https://raw.githubusercontent.com/hihihidoraemon/2026-4-1_Adv_daily_report/main/20260401_adv_report_template.xlsx"
# 页面基础配置
st.set_page_config(
    page_title="广告数据分析工具",
    page_icon="📊",
...
def calculate_budget_fluctuation(sheets,offer_base_info):
    ...
    # 仅对明确为金额的列转数值；文本/百分比字符串列不可 to_numeric，否则会整列变 NaN
    _skip_numeric = {
        "Advertiser", "appid", "country", "Payin",
        "历史最高利润对应日期",
    }
    for col in result_df.columns:
        if (
            col in _skip_numeric
            or "%" in col
            or "利润率" in col
            or "总结" in col
            or "类型" in col
